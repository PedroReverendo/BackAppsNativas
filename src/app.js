...
app.post("/producto", (req, res) => {
  const { ID_Equipo, Nombre, Descripcion, Precio, Talle, Foto, Stock, Temporada } = req.body;
  connection.query(
    'INSERT INTO Producto (ID_Equipo, Nombre, Descripcion, Precio, Talle, Foto, Stock, Temporada) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
    [ID_Equipo, Nombre, Descripcion, Precio, Talle, Foto, Stock, Temporada],
    (err, result) => {
      if (err) {
        console.error('Error executing query:', err);
        return res.status(500).send('Error executing query');
      }
      res.status(201).json({ message: 'Producto created', id: result.insertId });
    }
  );
});
